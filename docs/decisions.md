...
Still not live: no PostHog project is connected (no key exists to create one with), and no Vercel production deployment exists — an earlier attempt to create one via the Vercel API hit a 403 (the GitHub repo isn't connected to Vercel yet on Arthur's account), so Lighthouse numbers for `/`, `/quiz`, `/r/[id]` haven't been measured against a real deployment. `.env.example` documents what a deployment needs (Supabase URL/anon/service-role keys, PostHog key/host); the actual Supabase credentials were shared with Arthur directly rather than committed anywhere.
